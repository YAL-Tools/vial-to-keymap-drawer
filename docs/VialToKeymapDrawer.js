...
Main.convert_impl = function(forKeyRange) {
	...
	opt.layoutName = StringTools.trim(Main.fdLayout.value);
	...
	if(!forKeyRange) {
		...
		opt.parseEncoderDefs(Main.fdEncoderDefs.value);
...
VilToDrawer.postProcLayers = function(vLayers,opt) {
	var _g = 0;
	var _g1 = opt.encoderDefs;
	while(_g < _g1.length) {
		var encDef = _g1[_g];
		++_g;
		var _g2 = 0;
		while(_g2 < vLayers.length) {
			var vLayer = vLayers[_g2];
			++_g2;
			var _g3 = 0;
			var _g4 = [];
			var _g5 = 0;
			...
			while(_g5 < _g6.length) {
				var v = _g6[_g5];
				++_g5;
				if(v.col == encDef.col && v.row == encDef.row) {
					_g4.push(v);
				}
			}
			var _g7 = _g4;
			while(_g3 < _g7.length) {
				var ki = _g7[_g3];
				++_g3;
...
VilToDrawer.runTxt = function(opt) {
	...
	while(_g_current < _g_array.length) {
		...
		while(_g_current1 < _g_array1.length) {
			...
			var k = _g_key1;
			var kc = _g_value1;
			var dk = vial_VialKey.toDrawerKey(kc.key,opt,kc.encoder != null);
			dk = drawer_DrawerKey.postproc(dk,opt);
			var mo = "MO(" + li + ")";
			var lts = "LT" + li + "(";
			var held = false;
			var _g = 0;
			...
			if(kc.encoder != null && isVial) {
				...
				var encDn = drawer_DrawerKey.toFlat(drawer_DrawerKey.postproc(vial_VialKey.toDrawerKey(encPair[0],opt,true),opt),"t");
				var encUp = drawer_DrawerKey.toFlat(drawer_DrawerKey.postproc(vial_VialKey.toDrawerKey(encPair[1],opt,true),opt),"t");
			...
			if(opt.showKeyPos) {
				var dkx2 = drawer_DrawerKey.toExt(dk);
				dkx2.s = kc.row + "," + kc.col;
				dk = dkx2;
			}
			dkeys.push(dk);
		}
	...
	var extraStyle = [];
	var usesEncoders1 = usesEncoders;
	...
	if(vkm.combo != null && opt.combos) {
		...
		while(_g < _g1.length) {
			...
			while(_g_current < _g_array.length) {
				...
				var vKeys = _g_value;
				var ln = opt.getLayerName(li,true);
				if(!Object.prototype.hasOwnProperty.call(dkLayers,ln)) {
					continue;
				}
				var keyPos = [];
				var _g4 = 0;
				...
				var dk = vial_VialKey.toDrawerKey(cResult,opt);
				dk = drawer_DrawerKey.postproc(dk,opt);
				dCombos.push({ p : keyPos, k : dk, l : [ln]});
			}
		}
	}
	...
	if(opt.layoutName != null && opt.layoutName != "") {
		dkm.layout.layout_name = opt.layoutName;
	}
	if(opt.yamlLike) {
...
VilToDrawerOpt.prototype = {
	...
	,log: function(level,v) {
		haxe_Log.trace("[" + level + "]",{ fileName : "src/VilToDrawerOpt.hx", lineNumber : 43, className : "VilToDrawerOpt", methodName : "log", customParams : [v == null ? "null" : Std.string(v)]});
	}
	,info: function(v) {
		this.log("info",v);
	}
	...
	,parseKeyOverrides: function(txt) {
		...
		tools_ERegTools.each(VilToDrawerOpt.rxKeyOverrideName,txt,function(rx) {
			var key;
			try {
				key = JSON.parse(rx.matched(1));
			} catch( _g ) {
				var x = haxe_Exception.caught(_g).unwrap();
				_gthis.error("Couldn't parse " + rx.matched(1) + ": " + Std.string(x));
				return;
			}
			var this1 = _gthis.keyNames;
			var k = key.toLowerCase();
			var v = rx.matched(2);
			this1.h[k] = v;
		});
	}
	,parseEncoderDefs: function(txt) {
		var _gthis = this;
		tools_ERegTools.each(VilToDrawerOpt.rxEncoderDef,txt,function(rx) {
			_gthis.encoderDefs.push({ row : Std.parseInt(rx.matched(1)), col : Std.parseInt(rx.matched(2)), index : Std.parseInt(rx.matched(3)), rule : rx.matched(0)});
		});
	}
...
drawer_DrawerKey.postproc_1 = function(this1,s,opt) {
	if(s == null) {
		return null;
	}
	var lqs = s.toLowerCase();
	if(Object.prototype.hasOwnProperty.call(opt.keyNames.h,lqs)) {
		return opt.keyNames.h[lqs];
	} else {
		return s;
	}
};
drawer_DrawerKey.postproc = function(this1,opt) {
	if(drawer_DrawerKey.isFlat(this1)) {
		return drawer_DrawerKey.postproc_1(this1,this1,opt);
	}
	var ext = this1;
	if(ext.s != null && StringTools.endsWith(ext.s,"+")) {
		var lqs = (ext.s + ext.t).toLowerCase();
		if(Object.prototype.hasOwnProperty.call(opt.keyNames.h,lqs)) {
			ext.t = opt.keyNames.h[lqs];
			ext.s = "";
			return ext;
		}
	}
	if(ext.t != null) {
		ext.t = drawer_DrawerKey.postproc_1(this1,ext.t,opt);
	}
	if(ext.h != null) {
		ext.h = drawer_DrawerKey.postproc_1(this1,ext.h,opt);
	}
	if(ext.s != null) {
		ext.s = drawer_DrawerKey.postproc_1(this1,ext.s,opt);
	}
	return ext;
};
...
Main.buildDate = "2026-01-23--08-24-50";
...
Main.fdEncoderDefs = window.document.getElementById("encoder-defs");
...
VilToDrawerOpt.rxKeyOverrideName = new EReg("^\\s*(\".*?[^\\\\]\")\\s*=>\\s*(.+)","gm");
VilToDrawerOpt.rxEncoderDef = new EReg("^\\s*(\\d+),\\s*(\\d+)\\s*=>\\s*(\\d+)","gm");
